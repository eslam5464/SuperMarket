    ...
    class Backup
    {
        ...
        public async static Task AllWeekly(DayOfWeek Day, string strDestination = ".", string Id = "Default")
        {
            if (DateTime.Now.DayOfWeek == Day)
                await Task.Run(() => All(strDestination, Id, false));
        }
        ...
        public async static Task AllDaily(int NumOfMaxBackup = 10, string strDestination = ".", string Id = "Default")
        {
            ...
            foreach (var BackupFiles in new DirectoryInfo(BackupLocation).GetFiles().OrderByDescending(x => x.LastWriteTime).Skip(NumOfMaxBackup))
            {
                ...
                await Task.Run(() => Logger.Log($"Deleted file: '{BackupFiles.Name}' for reaching maximum backup of {NumOfMaxBackup}", System.Reflection.MethodInfo.GetCurrentMethod().Name, "Backup", Logger.INFO));
            }
        }
        ...
        public async static Task All(string strDestination = ".", string Id = "Default", bool Overwrite = false)
        {
            if (!Directory.Exists(BackupLocation))
            {
                ...
                await Task.Run(() => Logger.Log("created the directory for backup", System.Reflection.MethodInfo.GetCurrentMethod().Name, "Backup", Logger.INFO));
            }
            ...
            if (Overwrite)
            {
                ...
                try
                {
                    ...
                    await Task.Run(() => Logger.Log("created backup and orverwrited the file", System.Reflection.MethodInfo.GetCurrentMethod().Name, "Backup", Logger.INFO));
                }
                catch (Exception ex)
                {
                    await Task.Run(() => Logger.Log("Error when creating backup with diferential trying without it & error: " + ex.Message, System.Reflection.MethodInfo.GetCurrentMethod().Name, "Backup", Logger.ERROR));
                    ...
                    {
                        await Task.Run(() => Logger.Log("Error again when creating backup with and without diferential & error: " + ex2.Message, System.Reflection.MethodInfo.GetCurrentMethod().Name, "Backup", Logger.WARNING));
                    }
                }
            }
            ...
            if (!Overwrite && !File.Exists(strDestination + FileName))
            {
                try
                {
                    ...
                    await Task.Run(() => Logger.Log("created backup without overwriting the file", System.Reflection.MethodInfo.GetCurrentMethod().Name, "Backup", Logger.INFO));
                }
                catch (Exception ex)
                {
                    await Task.Run(() => Logger.Log("Error when creating backup: " + ex.Message, System.Reflection.MethodInfo.GetCurrentMethod().Name, "Backup", Logger.ERROR));
                }
            }
            else
            {
